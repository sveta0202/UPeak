(function (root) {
  ...
  var SECTION_LABELS = {
    ...
    specifics: "Что можно попробовать сегодня",
    today: "Что сегодня",
  ...
  var STATE_OVERRIDES = {
    emergency_recovery: {
      today: "Сон, энергия и стресс сегодня очень низкие — ресурса мало.",
      meaning: "Концентрация, терпение и качество решений сегодня, скорее всего, заметно ниже обычного.",
      consequence: "Если оставить день слишком плотным, к вечеру чаще накапливаются усталость и ошибки.",
      plan: {
        reduce: [
          "Можно оставить в списке только 1 важную задачу",
          "Всё несрочное можно отложить",
          "Можно лечь спать раньше обычного"
        ],
        leverage: []
      },
      why: "Когда сразу несколько ресурсов низкие, обычно лучше бережнее отнестись к дню и дать себе восстановиться.",
      benefit: "Так обычно легче сохранить ресурс и встретить завтра чуть спокойнее, без лишней накопленной усталости.",
      specifics: [
    ...
    mixed_severe: {
      today: "Одна зона сильно просела и тянет остальное вниз.",
      meaning: "Фокус и точность сегодня могут быть ниже обычного — особенно на сложных задачах.",
      consequence: "Если оставить план слишком плотным, усталость обычно накапливается быстрее.",
      plan: {
        reduce: [
          "Список можно сократить до 1–2 задач",
          "Лучше не пытаться закрыть всё",
          "Можно добавить короткую паузу между блоками"
        ],
        leverage: []
      },
      why: "Когда один ресурс сильно просел, короткий список обычно снижает нагрузку на внимание.",
      benefit: "Так обычно легче не потратить остаток ресурса на перегруз и спокойнее встретить завтра.",
      specifics: [
      ...
      if (axis === "sleep" && axisBlock.by_metric) {
        ...
        if (qualifiesSleepTune(dayState, "growth")) {
          axisBlock = Object.assign({}, axisBlock, {
            state_text: "Сон чуть короче идеала — вечером можно добавить отдыха, а днём начать со сложного блока."
          });
  ...
  function buildResult(entry, map) {
    ...
    return "Примерный ориентир по дню — примерно " + lo + "–" + hi + "%. Это не гарантия и зависит от задач.";
  }

  var RESULT_CONDITION = "Примерный ориентир (не гарантия):";
  ...
  function buildBenefit(entry, map) {
    ...
    if (map.mode === "recovery") {
      return "Так обычно легче сохранить ресурс и встретить завтра чуть спокойнее, без лишней накопленной усталости.";
    }
    if (map.mode === "growth") {
      return "Небольшое улучшение одной зоны часто помогает день пройти чуть ровнее и спокойнее.";
    }
    if (entry && entry.why) {
      return "Так обычно легче закрыть важное без ощущения перегруза.";
    }
    return "Так обычно легче закрыть важное без ощущения перегруза.";
  }
