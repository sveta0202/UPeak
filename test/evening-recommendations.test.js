...
// fatigue, taskStart, planOverload, detachment — совпадает с формой чек-ина.
function evening(fatigue, taskStart, planOverload, detachment) {
  return {
    fatigue: fatigue,
    taskStart: taskStart,
...
test("перегруз 5 без трудности старта не подменяет сигнал start_hard", function () {
...
test("усталость + трудность старта одновременно → fatigue побеждает, E3 не показывается", function () {
  var cards = recs(60, 5, 10, evening(2, 4, 4, 4));
  assert.equal(cards[0].decision_key, "start_hard");

  var tiredAndStartHard = recs(60, 2, 10, evening(4, 5, 5, 1));
  assert.equal(tiredAndStartHard[0].decision_key, "fatigue_high");
  var ids = tiredAndStartHard[0].embedOffers.map(function (o) { return o.id; });
  assert.equal(ids.indexOf("evening_tiny_start"), -1);
});

test("отключение 1 + трудность старта 5 (без усталости) → detachment_low, плашка одна (телефон)", function () {
